...
from backend.ingestion.video.schemas import VideoMetadata
...
class VideoIngestionService:
    ...
    def validate_and_ingest(self, file_source: Union[str, Path]) -> Tuple[Path, VideoMetadata]:
        ...
        max_file_size_mb = getattr(settings, 'MAX_FILE_SIZE_MB', 500.0)
        if file_size_mb > max_file_size_mb:
            raise VideoIngestionError(
                f"File size ({file_size_mb:.1f}MB) exceeds maximum allowed limit ({max_file_size_mb}MB)."
            )
    ...
    def _extract_metadata(self, video_path: Path, file_size_bytes: int) -> VideoMetadata:
        ...
        max_duration_seconds = getattr(settings, 'MAX_VIDEO_DURATION_SECONDS', 3600.0)
        if duration_seconds > max_duration_seconds:
            raise VideoIngestionError(
                f"Video duration ({duration_seconds:.1f}s) exceeds maximum allowed limit ({max_duration_seconds}s)."
            )
