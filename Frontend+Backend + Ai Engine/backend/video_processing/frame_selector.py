...
from backend.video_processing.schemas import VideoMetadata, SelectedFrame
...
class IntelligentFrameSelector:
    ...
    def select_keyframes(
        ...
        if duration <= settings.SHORT_VIDEO_THRESHOLD_SEC:
            target_fps = settings.SHORT_VIDEO_SAMPLING_FPS  # e.g., 1 frame per sec
        elif duration >= settings.LONG_VIDEO_THRESHOLD_SEC:
            target_fps = settings.LONG_VIDEO_SAMPLING_FPS   # e.g., 1 frame every 5 sec
        else:
            target_fps = settings.DEFAULT_SAMPLING_FPS      # e.g., 1 frame every 2 sec
        ...
        for t in timestamps_to_check:
            if len(selected_frames) >= settings.MAX_FRAMES_PER_ANALYSIS:
                break
            ...
            if frame_counter == 0:
                ...
                if score < (1.0 - settings.SCENE_CHANGE_THRESHOLD):
                    reason = "scene_change"
