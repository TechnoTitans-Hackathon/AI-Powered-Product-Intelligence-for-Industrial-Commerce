...
def test_full_end_to_end_pipeline_flow(client, db_session):
    ...
    assert len(updated_product.attributes) >= 1
    ...
    assert "confidenceLevel" in response_dto
    assert response_dto["confidenceLevel"] in ["HIGH", "MEDIUM", "LOW", "CONFLICT"]
    assert len(response_dto["dynamicAttributes"]) >= 1
