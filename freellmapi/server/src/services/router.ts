...
export function isModelEligible(entry: ChainRow, estimatedTokens: number, skipKeys?: Set<string>): string[] {
  const db = getDb();
  if (!hasProvider(entry.platform as Platform)) return ['no provider registered'];

  const allKeys = db.prepare(
    "SELECT * FROM api_keys WHERE platform = ? AND enabled = 1 AND status IN ('healthy', 'unknown')"
  ).all(entry.platform) as KeyRow[];
  if (allKeys.length === 0) return ['no enabled+healthy key for platform'];

  const keys = allKeys.filter(k => scopeAllows(parseModelScope(k.model_scope_json), entry.model_id));
  if (keys.length === 0) return ['no usable key — scoped to other models'];
  ...
  for (const key of keys) {
    ...
    // Found an eligible key
    return [];
  }

  return Object.entries(skipTally).map(([r, n]) => `${r}:${n}`);
}
...
export function routeRequest(estimatedTokens = 1000, skipKeys?: Set<string>, preferredModelDbId?: number, requireVision = false, requireTools = false, skipModels?: Set<number>, prefetchedChain?: ChainRow[], requireStructured = false, skipPlatforms?: Set<string>): RouteResult {
  ...
  const eligibleChain = chain.filter(entry => {
    const label = `${entry.platform}/${entry.model_id}`;

    if (skipModels?.has(entry.model_db_id)) { diag.push(`${label}: ruled out earlier this request`); return false; }
    if (skipPlatforms?.has(entry.platform)) { diag.push(`${label}: provider ruled out earlier this request`); return false; }
    if (requireVision && !entry.supports_vision) { diag.push(`${label}: no vision support`); return false; }
    if (requireTools && !entry.supports_tools) { diag.push(`${label}: no tool-calling support`); return false; }
    if (requireStructured && platformDropsResponseFormat(entry.platform)) { diag.push(`${label}: platform drops response_format`); return false; }
    if (entry.context_window != null && estimatedTokens > entry.context_window) { diag.push(`${label}: context ${entry.context_window} < estimated ${estimatedTokens}`); return false; }
    if (entry.tpm_limit != null && estimatedTokens > entry.tpm_limit) { diag.push(`${label}: tpm_limit ${entry.tpm_limit} < estimated ${estimatedTokens}`); return false; }

    const ineligibilityReasons = isModelEligible(entry, estimatedTokens, skipKeys);
    if (ineligibilityReasons.length > 0) {
      const summary = ineligibilityReasons.join(', ') || 'no usable key';
      diag.push(`${label}: ${summary}`);
      return false;
    }
  ...
  if (eligibleChain.length === 0) {
    throw new RouteError(`NO_ELIGIBLE_PROVIDER: ${summarizeExhaustion(diag, getSoonestCooldownExpiry())}`, 429, diag);
  }

  // STAGE 2: SCORING
  const sortedChain = orderChain(eligibleChain, strategy);
  ...
  if (preferredModelDbId) {
    ...
    if (idx >= 0) {
      ...
      if (pinnedRow) {
        const ineligibilityReasons = isModelEligible(pinnedRow, estimatedTokens, skipKeys);
        if (ineligibilityReasons.length === 0) {
          sortedChain.unshift(pinnedRow);
        } else {
          diag.push(`${pinnedRow.platform}/${pinnedRow.model_id}: pinned but ineligible (${ineligibilityReasons.join(', ')})`);
        }
      }
    }
  }
  ...
  for (const entry of sortedChain) {
    ...
    // The models in sortedChain already passed the request-level gates during STAGE 1 ELIGIBILITY.
    // However, they are repeated here because `sortedChain` might contain a sticky `pinnedRow` 
    // or an exploratory probe that needs to be double-checked against these gates if we didn't 
    // fully vet them before unshifting (though we did above). It is safe and quick to check them again.
    // It also allows the code to easily fall through.

    // Request-level gates
    if (skipModels?.has(entry.model_db_id)) continue;
    if (skipPlatforms?.has(entry.platform)) continue;
    if (requireVision && !entry.supports_vision) continue;
    if (requireTools && !entry.supports_tools) continue;
    if (requireStructured && platformDropsResponseFormat(entry.platform)) continue;
